  ...
  useEffect(() => {
    ...
    evtSource.onmessage = (event) => {
      ...
      switch (data.type) {
        ...
        case 'participants': setParticipants(data.participants); break;
        case 'message': 
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-2 gap-2">
            {availableModels.map(m => (
              <label key={m} className="flex items-center space-x-2 p-2 border rounded hover:bg-gray-50">
                <input 
                  type="checkbox"
      ...
      <header className="p-4 border-b bg-white flex items-center justify-between">
        <div>
          ...
          <div className="text-sm text-gray-500 flex items-center space-x-2">
            <span className={`w-2 h-2 rounded-full ${status === 'discussing' ? 'bg-green-500' : 'bg-yellow-500'}`} />
        ...
        <div className="flex items-center space-x-2">
          <button onClick={togglePause} className="px-3 py-1 border rounded hover:bg-gray-50">
            {status === 'paused' ? 'Resume' : 'Pause'}
          </button>
          <button onClick={toggleMode} className="px-3 py-1 border rounded hover:bg-gray-50">
            Switch to {mode === 'auto' ? 'Manual' : 'Auto'}
          </button>
          <button onClick={stopChat} className="px-3 py-1 border rounded text-red-600 hover:bg-red-50">
            Stop
          </button>
        </div>
      </header>
        ...
        {messages.map(msg => {
          ...
          return (
            <div key={msg.id} className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[80%] rounded-lg p-4 ${isUser ? 'bg-blue-600 text-white' : 'bg-white border shadow-sm'}`}>
