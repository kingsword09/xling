...
export function ParticipantsSidebar({ sessionId, participants, mode, onClose }: ParticipantsSidebarProps) {
      ...
      <ScrollArea className="flex-1 p-4">
        <div className="space-y-4">
          {participants.map((participant) => (
            <div key={participant.id} className="flex items-center justify-between group rounded-lg border p-3 hover:bg-muted/50 transition-colors">
              <div className="flex items-center gap-3 overflow-hidden">
                <Avatar className="h-8 w-8 border">
                  <AvatarFallback className={participant.type === 'ai' ? "bg-primary/10 text-primary" : "bg-secondary text-secondary-foreground"}>
                    {participant.type === 'ai' ? <Bot className="h-4 w-4" /> : <User className="h-4 w-4" />}
                  </AvatarFallback>
                </Avatar>
                <div className="flex flex-col truncate">
                  <span className="text-sm font-medium truncate" title={participant.name}>
                    {participant.name}
                  </span>
                  {participant.model && (
                    <span className="text-xs text-muted-foreground truncate" title={participant.model}>
                      {participant.model}
                    </span>
                  )}
                </div>
        ...
        <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
          ...
          <DialogContent>
            ...
            <DialogFooter>
              ...
              <Button onClick={handleAddParticipant} disabled={!selectedModel}>Add</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
