...
export default class DiscussCommand extends Command {
  ...
  async #runCLI(flags: Interfaces.InferredFlags<typeof DiscussCommand.flags>) {
    ...
    if (selectedModels.length === 0) {
      selectedModels = await this.#selectModelsInteractive(availableModels);
    }
    ...
    if (process.stdin.isTTY) {
      ...
      process.stdin.resume(); // ensure keypress events are delivered in raw mode
    }

    // Fallback to handle Ctrl+C when raw mode suppresses default SIGINT
    const handleSigint = () => {
      engine.stop();
      process.exit();
    };
  ...
  async #selectModelsInteractive(models: string[]): Promise<string[]> {
    if (models.length === 0) {
      return [];
    }
